...
void ValidateBounds(Number number_to_check, Number min_value,
	...
	if (number_to_check < min_value || number_to_check > max_value) {
		std::stringstream error;
		error << number_to_check << " is out of [" << min_value << ", "
				<< max_value << "]";
...
struct Request {
	enum class Type {
		COMPUTE_INCOME, EARN, SPEND, PAY_TAX
	};
...
const unordered_map<string_view, Request::Type> STR_TO_REQUEST_TYPE = { {
		"ComputeIncome", Request::Type::COMPUTE_INCOME }, { "Earn",
		Request::Type::EARN }, { "Spend", Request::Type::SPEND }, { "PayTax",
		Request::Type::PAY_TAX }, };
...
struct AddMoneyRequest: ModifyRequest {
	...
	void Process(BudgetManager& manager) const override {
		...
		const MoneyState daily_change = SIGN == 1 ? MoneyState { .earned =
				daily_value } :
													MoneyState { .spent =
															daily_value };
...
vector<double> ProcessRequests(const vector<RequestHolder>& requests) {
	...
	for (const auto& request_holder : requests) {
		if (request_holder->type == Request::Type::COMPUTE_INCOME) {
			...
			const auto& request =
					static_cast<const ModifyRequest&>(*request_holder);
